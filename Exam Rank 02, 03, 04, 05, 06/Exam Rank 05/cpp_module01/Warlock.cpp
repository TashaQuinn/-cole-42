...
Warlock::~Warlock() { 
    ...
    while (it_begin != it_end) {
        delete it_begin->second; // cuz we have ASpell * on which we allocated memory
        ++it_begin;
    }
...
void Warlock::forgetSpell(std::string spell_name) {
    ...
    if (it != arr.end())
        delete it->second; // cuz we have cloned spell obj on which we allocated memory in learnSpell
